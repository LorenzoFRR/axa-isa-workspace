...
print("• bronze resumo:", BRONZE_CORRETOR_RESUMO_FQN)
print("• silver resumo:", SILVER_CORRETOR_RESUMO_FQN, "(tabela fixa)\n")

print("• bronze detalhe:", BRONZE_CORRETOR_DETALHE_FQN)
...
def apply_rules_for_table(
    ...
    for r in rules:
        ...
        cols_before = set(df_out.columns)
        try:
            df_out = r["fn"](df_out)
        except Exception as e:
            raise RuntimeError(f"Falha ao aplicar regra {table_key}.{rid}: {desc}. Erro: {e}") from e
        cols_after = set(df_out.columns)

        exec_log.append({
            "rule_id":     rid,
            "status":      "APPLIED",
            "description": desc,
            "cols_dropped": sorted(cols_before - cols_after),
            "cols_added":   sorted(cols_after  - cols_before),
        })
...
def GEN_R00_uppercase_cols(df: DataFrame) -> DataFrame:
    # Garante que todos os nomes de coluna estejam em UPPERCASE antes das demais regras.
    # Preserva colunas técnicas adicionadas pela ingestão (TS_ARQ, FL_NOVO, etc.) que já são uppercase.
...
def DET_R04_uppercase_cols(df: DataFrame) -> DataFrame:
    return df.toDF(*[c.upper() for c in df.columns])
...
RULES_BY_TABLE = {
    "cotacao_generico": [
        rule_def("GEN_R00", "Padronizar nomes de colunas para UPPERCASE antes das demais regras",
                 GEN_R00_uppercase_cols, enabled=True),
    ...
    "corretor_resumo": [
        ...
        # RES_R05 desabilitada em inferência: dados de DADOS_ANTIGOS podem ter colunas novas
        # 100% nulas porque são anteriores à criação dessas colunas na fonte. Não queremos
        # que o conteúdo do batch atual dite o schema silver — as colunas devem ser preservadas.
        rule_def("RES_R05", "Dropar colunas 100% vazias (NULL ou '') — DESABILITADA em inferência",
                 RES_R05_drop_cols_100pct_vazias, enabled=False),
    ...
    "corretor_detalhe": [
        ...
        rule_def("DET_R03", "Converter HR_* para decimal(17,6)", DET_R03_hr_decimals,
                 enabled=True),
        rule_def("DET_R04", "Garantir nomes de colunas em UPPERCASE",
                 DET_R04_uppercase_cols, enabled=True),
    ],
}
            ...
            cols_impact[tbl] = [
                {
                    ...
                    "status":      entry["status"],
                    "cols_dropped": entry.get("cols_dropped", []),
                    "cols_added":   entry.get("cols_added", []),
                }
                for entry in logs
                if entry.get("cols_dropped") or entry.get("cols_added")
            ]
            # print para visibilidade no notebook
            drops = [(e["rule_id"], e["cols_dropped"]) for e in logs if e.get("cols_dropped")]
            if drops:
                print(f"\n[{tbl}] colunas dropadas por regra:")
                for rid, dropped in drops:
                    print(f"  {rid}: {dropped}")
        mlflow.log_dict(cols_impact, "cols_impact.json")
