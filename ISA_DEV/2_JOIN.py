...
# =========================
SILVER_FACT_FQN   = "silver.cotacao_generico_clean_20260325_163322"      # <<< AJUSTE
SILVER_RESUMO_FQN = "silver.corretor_resumo_clean_20260325_163322"       # <<< AJUSTE
SILVER_DETALHE_FQN= "silver.corretor_detalhe_clean_20260325_163322"      # <<< AJUSTE
...
def normalize_key(col: F.Column) -> F.Column:
    """
    Normalização padrão de chaves:
    - Chaves numéricas (long, double, string de dígitos): normaliza via long
      → remove zeros à esquerda e trata notação científica de doubles
      (ex: double "9.2890001E9" → long 9289000100 → string "9289000100")
    - Chaves textuais (DS_PRODUTO_NOME, DS_TIPO_COTACAO, etc.): cast para long
      retorna null → fallback para string, comportamento inalterado
    - "" → NULL
    """
    c_as_long = col.cast("long")
    c_str = F.trim(col.cast("string"))
    c = F.when(
        c_as_long.isNotNull() & c_str.isNotNull() & (c_str != F.lit("")),
        c_as_long.cast("string")
    ).otherwise(c_str)
    # Defensivo: strip de ".0" residual para casos não capturados pelo long
    c = F.regexp_replace(c, r"^(-?\d+)\.0+$", "$1")
