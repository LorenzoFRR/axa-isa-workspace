...
TOPK_LIST = [5]  # <<< AJUSTE — feature sets gerados pelo FS (usados em TREINO_FEATURE_SET_KEY)

# =========================
# TREINO — params
# =========================
# Chave do feature set gerado pelo FS — deve existir em FS_FEATURE_SETS após T_FS.
...
print("• fs_seeds            :", FS_SEEDS)
print("• topk_list           :", TOPK_LIST)
print("• treino_feature_set  :", TREINO_FEATURE_SET_KEY)
print("• treino_pinned       :", TREINO_FEATURES_PINNED)
...
_pp_step_ctx = (
    mlflow.start_run(run_id=STEP_PRE_PROC_RUN_ID_OVERRIDE, nested=True)
    if STEP_PRE_PROC_RUN_ID_OVERRIDE
    else mlflow.start_run(run_name=STEP_PRE_PROC_NAME, nested=True)
)
with _pp_step_ctx as _pp_container:
...
_explore_step_ctx = (
    mlflow.start_run(run_id=STEP_CLF_EXPLORE_RUN_ID_OVERRIDE, nested=True)
    if STEP_CLF_EXPLORE_RUN_ID_OVERRIDE
    else mlflow.start_run(run_name=STEP_CLF_EXPLORE_NAME, nested=True)
)
with _explore_step_ctx as _clf_explore_container:
...
_fit_step_ctx = (
    mlflow.start_run(run_id=STEP_CLF_FIT_RUN_ID_OVERRIDE, nested=True)
    if STEP_CLF_FIT_RUN_ID_OVERRIDE
    else mlflow.start_run(run_name=STEP_CLF_FIT_NAME, nested=True)
)
with _fit_step_ctx as _clf_fit_container:
...
from pyspark.ml.functions import vector_to_array as _vta_fs
...
def compute_ap(df_pred: DataFrame, label_col: str = "label", prob_col: str = "probability") -> float:
    p1    = _vta_fs(F.col(prob_col)).getItem(1).cast("double")
...
_fs_kw = {"run_id": STEP_FS_RUN_ID_OVERRIDE} if STEP_FS_RUN_ID_OVERRIDE else {"run_name": STEP_FS_NAME}
...
with mlflow.start_run(**_fs_kw, nested=True) as fs_container:
    ...
    with mlflow.start_run(run_name=RUN_FS_EXEC, nested=True):
        ...
        # DF_MODEL_FQN é lido do gold — já contém CLF_CORRETOR adicionada por T_CLUSTERING_FIT.
        # FEATURE_CANDIDATES com CLF_CORRETOR=True controla se ela entra no pipeline.
        ...
        DROP_FEATURES_FINAL = set(_drop_base_fs) | drop_constant_cols | EXCLUIR_DE_FEATURES
        ...
        for seed in FS_SEEDS:
            ...
            # Remove columns with all nulls from NUM_COLS_FINAL before Imputer
            non_null_num_cols = [
                c for c in NUM_COLS_FINAL
                if df_tr_ml.filter(F.col(c).isNotNull()).count() > 0
            ]
            imp_cols = [f"{c}__imp" for c in non_null_num_cols]
            ...
            if idx_cols:
                pp_stages.append(OneHotEncoder(inputCols=idx_cols, outputCols=ohe_cols, dropLast=False))
            if non_null_num_cols:
                pp_stages.append(Imputer(
                    inputCols=non_null_num_cols,
                    outputCols=imp_cols,
                    strategy="mean"
                ))
            pp_stages.append(VectorAssembler(inputCols=ohe_cols + imp_cols, outputCol="features_vec"))    
        ...
        # ── [9] Correlação de Pearson (exploratória) ──────────────────────────────
        ...
        for k in TOPK_LIST:
            print(f"• top_{k}      :", FS_FEATURE_SETS.get(f"top_{k}"))
...
_tr_kw = {"run_id": STEP_TREINO_RUN_ID_OVERRIDE} if STEP_TREINO_RUN_ID_OVERRIDE else {"run_name": STEP_TREINO_NAME}
...
with mlflow.start_run(**_tr_kw, nested=True) as treino_container:
    ...
    with mlflow.start_run(run_name=RUN_TREINO_EXEC, nested=True) as treino_exec_run:
        ...
        treino_cat_cols = [c for c in TREINO_FEATURE_COLS if c in FS_CAT_COLS_FINAL]
        ...
        mlflow.log_params({
            ...
            "note_mode_d":             "todos_combos_salvos_sem_selecao_vencedor",
        })
